defmodule Electric.Plug.ServeShapePlugTest do
  ...
  describe "ServeShapePlug" do
    ...
    test "honours send_deltas query param for shape", _cxt do
      test_shape = %Shape{
        root_table: {"public", "users"},
        table_info: %{
          {"public", "users"} => %{
      ...
      test_shape_id = "test-shape-without-deltas"
      next_offset = LogOffset.increment(@first_offset)
      ...
      conn =
        conn(:get, %{"root_table" => "public.users"}, "?offset=-1&send_deltas=false")
        |> ServeShapePlug.call([])
